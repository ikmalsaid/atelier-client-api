...
def AtelierWebAPI(client, host: str = "0.0.0.0", port: int = 5733, debug: bool = False):
    ...
    try:
        ...
        @app.route('/v1/api/image/consistent', methods=['POST'])
        def image_consistent_api():
            """
            Handle consistency requests via form data and files.
            
            Form Parameters:
            - prompt (str, required): User's positive prompt
            - negative_prompt (str, optional): User's negative prompt
            - image_size (str, optional): Desired image size ratio (default: "1:1")
            - face_strength (float, optional): Strength of face consistency (default: 1.2)
            - style_strength (float, optional): Strength of style application (default: 0.7)
            - image_seed (int, optional): Seed for image generation (default: 0)
            - style_name (str, optional): Name of the style preset (default: "none")
            
            Files:
            - face_image (file, required): Face reference image
            - style_image (file, required): Style reference image
            """
            try:
                if 'face_image' not in request.files and 'style_image' not in request.files:
                    raise Exception("At least one image (face or style) must be provided")

                data = {
                    'prompt': request.form.get('prompt'),
                    'negative_prompt': request.form.get('negative_prompt', ''),
                    'image_size': request.form.get('image_size', '1:1'),
                    'face_strength': request.form.get('face_strength', 1.2),
                    'style_strength': request.form.get('style_strength', 0.7),
                    'image_seed': request.form.get('image_seed', 0),
                    'style_name': request.form.get('style_name', 'none')
                }

                # Only add images that were provided
                if 'face_image' in request.files:
                    data['face_image'] = request.files['face_image']
                if 'style_image' in request.files:
                    data['style_image'] = request.files['style_image']

                if not data['prompt']:
                    raise Exception("Missing prompt")

                result = client.image_consistent(**data)
                if not result:
                    raise Exception("Consistency generation failed")
                ...
                client.logger.error(f"Error in image_consistent_api: {e}")
                return jsonify({"success": False, "error": str(e)}), 400

        @app.route('/v1/api/face/identity', methods=['POST'])
        def face_identity_api():
            """
            Handle face identity requests via form data and files.
            
            Form Parameters:
            - prompt (str, required): User's positive prompt
            - negative_prompt (str, optional): User's negative prompt
            - image_size (str, optional): Desired image size ratio (default: "1:1")
            - strength (float, optional): Strength of face consistency (default: 1.0)
            - image_seed (int, optional): Seed for image generation (default: 0)
            - style_name (str, optional): Name of the style preset (default: "none")
            
            Files:
            - image (file, required): Face reference image
            """
            try:
                ...
                result = client.face_identity(**data)
                if not result:
                    raise Exception("Face identity generation failed")
                ...
                client.logger.error(f"Error in face_identity_api: {e}")
                return jsonify({"success": False, "error": str(e)}), 400
        ...
        def api_index():
            ...
            return render_template('__idx__.py')
